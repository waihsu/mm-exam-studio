...
function getStudyInsight(sessions: PracticeSessionSummary[]) {
  ...
  return { completed, average, weakest };
}
...
export function DashboardPage() {
  ...
  const { completed, average, weakest } = getStudyInsight(practiceSessions);
  ...
  return (
    <div className="space-y-7 pb-4">
      ...
      <section className="relative isolate overflow-hidden rounded-[28px] bg-[#10182d] text-white shadow-[0_28px_70px_-42px_rgba(15,23,42,0.85)]">
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_82%_16%,rgba(114,96,255,0.43),transparent_29%),radial-gradient(circle_at_14%_110%,rgba(15,166,184,0.2),transparent_38%)]" />
        ...
        <div className="relative p-6 sm:p-8 lg:p-10">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <span className="rounded-full border border-white/10 bg-slate-950/20 px-3 py-1.5 text-xs font-medium text-slate-300">Open access</span>
          </div>
          <div className="mt-10 grid gap-10 lg:grid-cols-[minmax(0,1.45fr)_minmax(270px,0.55fr)] lg:items-end">
          <div className="max-w-3xl">
            <p className="ui-kicker flex items-center gap-2 text-indigo-200"><Sparkles className="h-4 w-4" /> Today’s study brief</p>
            ...
            <h2 className="mt-4 max-w-2xl text-[clamp(1.25rem,2.3vw,1.75rem)] font-semibold leading-snug tracking-[-0.025em] text-indigo-100">{focusTitle}</h2>
            <p className="mt-4 max-w-xl text-[0.9375rem] leading-7 text-slate-300">{focusDescription}</p>
            <div className="mt-8 flex flex-wrap gap-3">
              {activeSession ? (
                <Button asChild className="h-11 rounded-xl bg-white px-5 text-slate-950 shadow-lg shadow-black/10 hover:bg-indigo-50"><Link to="/practice/$sessionId" params={{ sessionId: activeSession.id }}><CirclePlay className="h-4 w-4" /> Continue practice</Link></Button>
              ) : (
                <Button asChild className="h-11 rounded-xl bg-white px-5 text-slate-950 shadow-lg shadow-black/10 hover:bg-indigo-50"><Link to="/practice">Start focused practice <ArrowRight className="h-4 w-4" /></Link></Button>
              )}
      ...
      <section className="grid gap-4 lg:grid-cols-[minmax(0,1.28fr)_minmax(280px,0.72fr)]">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_18px_45px_-38px_rgba(15,23,42,0.36)] sm:p-6">
          <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="ui-kicker text-indigo-700">Choose your lane</p><h2 className="mt-2 text-xl font-bold tracking-tight text-slate-950">One clear purpose for every study mode.</h2></div><Target className="h-5 w-5 text-indigo-600" /></div>
        ...
        <section className="rounded-2xl border border-indigo-100 bg-indigo-50/70 p-5 sm:p-6"><p className="ui-kicker text-indigo-700">Your study signal</p><h2 className="mt-2 text-xl font-bold tracking-tight text-slate-950">{weakest ? `${weakest.label} needs another pass.` : "Your feedback loop starts with one session."}</h2><p className="mt-3 text-sm leading-6 text-slate-600">{weakest ? "A fresh, focused attempt is more useful than trying to cover everything at once." : "Finish a session and the most useful next focus will appear here."}</p><Link to="/practice" className="mt-6 inline-flex items-center gap-1.5 text-sm font-bold text-indigo-700 transition hover:gap-2.5 hover:text-indigo-950">Choose a subject <ArrowRight className="h-4 w-4" /></Link></section>
      </section>

      {isNewWorkspace ? (
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_20px_52px_-42px_rgba(15,23,42,0.45)]">
          <div className="flex flex-col gap-5 border-b border-slate-100 px-5 py-5 sm:flex-row sm:items-end sm:justify-between sm:px-7 sm:py-6">
            <div>
              <p className="ui-kicker text-indigo-700">Your first 10 minutes</p>
              <h2 className="mt-2 text-xl font-bold tracking-tight text-slate-950">Start with one focused result—not every feature.</h2>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">This is the shortest path from a new account to a useful study signal.</p>
            </div>
            <span className="w-fit rounded-full bg-indigo-50 px-3 py-1.5 text-xs font-bold text-indigo-700">Guided start</span>
          </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)]">
              ...
              {recentCompleted.map((session) => (
                <Link key={session.id} to="/practice/$sessionId" params={{ sessionId: session.id }} className="group flex flex-wrap items-center justify-between gap-4 py-4 first:pt-0 last:pb-0">
                  <div className="min-w-0">
                    <p className="font-semibold text-slate-950 transition group-hover:text-indigo-700">{session.title || "Practice session"}</p>
                    <p className="mt-1 text-sm leading-6 text-slate-500">{scopeLabel(session)} · {session.totalQuestions} questions · {formatDate(session.completedAt)}</p>
                  </div>
                  <div className="flex items-center gap-4">
                    <p className="text-right text-base font-extrabold text-slate-950">{session.scorePercent ?? 0}%<span className="mt-0.5 block text-xs font-medium text-slate-400">{session.correctAnswers}/{session.totalQuestions} correct</span></p>
                    <ArrowRight className="h-4 w-4 text-slate-300 transition group-hover:translate-x-0.5 group-hover:text-slate-900" />
                  </div>
                </Link>
              ))}
            </div>
          ) : (
            <div className="py-3"><p className="text-base font-semibold text-slate-900">No completed practice yet.</p><p className="mt-2 max-w-lg text-sm leading-6 text-slate-500">Start with one grade and subject. Your result will be saved here once you finish.</p><Button asChild className="mt-5"><Link to="/practice">Start practice <ArrowRight className="h-4 w-4" /></Link></Button></div>
          )}
        </SectionCard>
        ...
        <SectionCard title="Workspace at a glance" description="Useful limits and account information—kept out of your way.">
          ...
          <div className="mt-5 border-t border-slate-100 pt-4 text-sm font-semibold text-slate-600"><span className="inline-flex items-center gap-2"><TrendingUp className="h-4 w-4 text-indigo-600" /> Study consistently, not endlessly.</span></div>
        </SectionCard>
      </div>
    </div>
  );
...
function HeroMetric({ label, value }: { label: string; value: string }) {
  return <div className="min-w-0 bg-slate-950/35 px-4 py-5 backdrop-blur-sm"><p className="truncate text-2xl font-extrabold tracking-tight text-white sm:text-[1.75rem]">{value}</p><p className="mt-1 text-[0.625rem] font-bold uppercase tracking-[0.13em] text-indigo-200">{label}</p></div>;
}
...
function ActionTile({ icon: Icon, title, description, to, action }: { icon: typeof BookOpenCheck; title: string; description: string; to: "/practice" | "/question-papers/new"; action: string }) {
  return <Link to={to} className="group rounded-xl border border-slate-200 bg-slate-50/70 p-4 transition hover:-translate-y-0.5 hover:border-indigo-300 hover:bg-white hover:shadow-[0_16px_30px_-26px_rgba(15,23,42,0.38)]"><span className="flex h-9 w-9 items-center justify-center rounded-lg bg-white text-indigo-700 shadow-sm ring-1 ring-slate-200"><Icon className="h-[18px] w-[18px]" /></span><h3 className="mt-4 text-[1rem] font-bold leading-6 text-slate-950">{title}</h3><p className="mt-1.5 text-sm leading-6 text-slate-500">{description}</p><span className="mt-4 inline-flex items-center gap-1 text-sm font-bold text-indigo-700">{action}<ArrowRight className="h-4 w-4 transition group-hover:translate-x-0.5" /></span></Link>;
}
...
function OnboardingStep({
  ...
  return (
    <div className="relative p-5 sm:p-6">
      <span className="text-xs font-extrabold tracking-[0.16em] text-indigo-600">{number}</span>
      <h3 className="mt-4 text-base font-bold text-slate-950">{title}</h3>
      <p className="mt-2 text-sm leading-6 text-slate-600">{description}</p>
      <Link to={to} className="mt-5 inline-flex items-center gap-1.5 text-sm font-bold text-indigo-700 transition hover:gap-2.5 hover:text-indigo-950">
...
function DashboardListSkeleton() {
  return (
    <div className="space-y-4" aria-label="Loading recent practice">
      {[0, 1, 2].map((item) => (
        <div key={item} className="flex items-center justify-between gap-5 border-b border-slate-100 pb-4 last:border-b-0 last:pb-0">
          <div className="space-y-2"><div className="h-4 w-40 animate-pulse rounded bg-slate-100" /><div className="h-3 w-56 animate-pulse rounded bg-slate-100" /></div>
